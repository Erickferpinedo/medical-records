...
with open('all_drug_side_effects.txt', 'w', encoding='utf-8') as output_file:
    for entry in drug_info_A:
        ...
        if start_index != -1 and end_index != -1:
            ...
            # Split into sentences and remove the third sentence
            sentences = side_effects.split('. ')
            if len(sentences) >= 3:
                # Remove the third sentence (index 2)
                del sentences[2]
            # Join the remaining sentences back together
            filtered_side_effects = '. '.join(sentences).strip()
